...
def pretraining_prints(p_index, lr, stepovi, lambda_parametri, batch_size, loss_type, net_type,jupyter):
    ...
    logs_path = base_folder_path + "/logs/" + script_name + "/" + str(
        p_index) + "_" + today + "_" + net_type + "_lr_" + str(lr) + "_step_na_" + str(
        stepovi) + "_epoha_" + "lambda_" + str(lambda_parametri) + "_batch_size_" + str(batch_size) + "_sched_" + str(
        scheduler_lr) + "_loss_" + loss_type  # path to the folder that we want to save the logs for Tensorboard
    if not os.path.exists(logs_path):
        os.mkdir(logs_path)
...
def after_data_loading_prints(lr, ime_foldera_za_upis, train_loader, valid_loader, test_loader):
    ...
    ispis = ("Size of Test dataset : ", str(len(test_loader.dataset.img_names)))
    print(ispis)
    upisivanje(ispis, ime_foldera_za_upis)
...
def end_of_epoch_print(epoch, all_train_losses, all_validation_losses, all_test_losses, optimizer, ime_foldera_za_upis):
    ispis = "Epoch: " + str(epoch) + " ; " + "Train Loss = " + str(all_train_losses[epoch]) + \
            " Validation Loss = " + str(all_validation_losses[epoch]) + " Test Loss = " + str(
        all_test_losses[epoch]) + " Learning rate = " + str(
